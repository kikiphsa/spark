...
/**
  ...
  */
case class MongoBDConfig(val uri: String, var db: String)
...
/**
  ...
  * @param r   Movie的评分
  */
case class Recommender(rid: Int, r: Double)
...
/**
  ...
  */
case class GenresRecommender(genres: String, recs: Seq[Recommender])
...
object StatisticsRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mongoBDConfig = MongoBDConfig(config("mongo.uri"), config("mongo.db"))
    ...
    //统计所有历史数据中每个电影的评分数
    //数据结构 -》  mid,count
    val rateMoreMoviresDF = spark.sql("select mid,count(mid) as count from ratings group by mid")

    println(rateMoreMoviresDF.count())
    //写入到mongodb中
    ...
    val ratingOfYearmouth = spark.sql("select mid, score ,changData(timestamp) as yeamouth from ratings")
    // 将新的数据集注册成为一张表
    ratingOfYearmouth.createOrReplaceTempView("ratingOfMouth")

    val rateMovieRecentlyMovies = spark.sql("select mid,count(mid) as count, yeamouth from ratingOfMouth group by yeamouth,mid")
    ...
    //计算电影类别top10
    val genresTOPMoives = genresRDD.cartesian(movieWithScore.rdd)
      .filter {
      ...
      .map {
        // 通过评分的大小进行数据的排序，然后将数据映射为对象 (26425,3.5), (32460,5.0)
        case (genres, items) => GenresRecommender(genres, items.toList.sortWith(_._2 > _._2).take(10).map(item => Recommender(item._1, item._2)))
      }.toDF()

    genresTOPMoives.take(3) foreach (println)
