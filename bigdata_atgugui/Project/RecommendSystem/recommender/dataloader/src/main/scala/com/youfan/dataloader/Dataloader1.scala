...
case class Rating(val mid: Int, val uid: Int, val source: Double, val timestamp: String)
...
object Dataloader {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ratingDF = ratingRDD.map(item => {
      val attr = item.split(",")
      Rating(attr(0).toInt, attr(1).toInt, attr(2).toDouble, attr(3).trim)
    }).toDF()
    ...
    /**
      ...
      */
    val newTag = tagDF.groupBy($"mid").agg(concat_ws("|", collect_set($"tag"))
