...
/**
  * Create by chenqinping on 2019/4/15 8:43
  * 离线计算
  */
/**
  * Movie数据集，数据集字段通过分割
...
object offlineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //创建一个SparkConf配置 executor 执行需要的内存 memory 需要的内存
    ...
    ratingRDD.take(2).foreach(println)

    //(370,5349,4.5,1096512187)


    //用户的数据集 RDD[Int] 获取了用户的id
    val userRDD = ratingRDD.map(_._1).distinct()
    //电影数据集 RDD[Int] //获取电影id
    ...
    movieRDD.take(2).foreach(println)
    ...
    //计算用户推荐矩阵
    //需要构造一个usersProducts  RDD[(Int,Int)] //笛卡儿积
    ...
    preRatings.take(2).foreach(println)

    /**
      * Rating(312,43832,1.2683787264704172)
      * Rating(296,43832,0.6593367368819242)
      */
    ...
    userRecs.take(3).foreach(println)
    ...
    movieFeatures.take(3).foreach(println)
    ...
    movieRecs.take(3).foreach(println)
